...
func Test_should_upscale_minutes_into_nanoseconds(t *testing.T) {
	buffer := make([]byte, 32)
	lp := NewLineParser(buffer, "m")
	reader := bytes.NewBuffer([]byte("foo,x=y value=1 1\n"))

	line, err := lp.Next(reader)
	require.Equal(t, "foo,x=y value=1 60000000000", string(line))
	require.Nil(t, err)
...
func Test_should_upscale_hours_into_nanoseconds(t *testing.T) {
	buffer := make([]byte, 32)
	lp := NewLineParser(buffer, "h")
	reader := bytes.NewBuffer([]byte("foo,x=y value=1 1\n"))

	line, err := lp.Next(reader)
	require.Equal(t, "foo,x=y value=1 3600000000000", string(line))
	require.Nil(t, err)
...
func Test_should_upscale_days_into_nanoseconds(t *testing.T) {
	buffer := make([]byte, 32)
	lp := NewLineParser(buffer, "d")
	reader := bytes.NewBuffer([]byte("foo,x=y value=1 1\n"))

	line, err := lp.Next(reader)
	require.Equal(t, "foo,x=y value=1 86400000000000", string(line))
	require.Nil(t, err)
...
func Test_should_upscale_weeks_into_nanoseconds(t *testing.T) {
	buffer := make([]byte, 32)
	lp := NewLineParser(buffer, "w")
	reader := bytes.NewBuffer([]byte("foo,x=y value=1 1\n"))

	line, err := lp.Next(reader)
	require.Equal(t, "foo,x=y value=1 604800000000000", string(line))
	require.Nil(t, err)
